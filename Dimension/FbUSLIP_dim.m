function [nstep,y_tot,y_punt_pol,t_punt_pol,t_event,xt_tot,y_event,epsilon,conv]=FbUSLIP_dim(Pin,Cin,IC,epsilon)
% ---------------------- Variables extraction and initialization ---------
% global variables
global m M k rhow g V r0 drmax Ax Ay cdx cdy rs alfa
% global additional variables 
global xt t0e current_e rho rhodot tspan2
...
maxstep=length(epsilon);  
    ...
    for nstep=1:maxstep
        ...
        rho = [];
        rhodot = [];
        [rho, rhodot] = FbRadiusPuntPhaseDim(tspan2,YE);
      
        options = odeset('RelTol',1e-9,'AbsTol',1e-9,'Events',@eventsLOdisturbedPol);
        [T2,Y2,TE2,YE2,IE2]=ode113(@(t,y) FbThetaPuntPhaseDim(t,y,rho,rhodot,tspan2),t,YE(3:4),options);
        %formattazione dati in uscita
        rho = interp1(tspan2, rho, T2); % get the rho(_dot) data for the time steps of interest
        rhodot = interp1(tspan2, rhodot, T2); % cut by the event handler in ode
        Y2 = [rho rhodot Y2];
        tmp = interp1(T2, rho, TE2);
        tmp2 = interp1(T2, rhodot, TE2);
        YE2 = [tmp, tmp2, YE2];
...
function [value,isterminal,direction] = eventsLOdisturbedPol(t,y) 
global r0 r rho tspan2
rho_t = interp1(tspan2, rho, t); % Interpolate the data set (ft, f) at times t
value = [rho_t-(r0+r),rho_t*sin(y(1))]; % detect height = 0, originale usa drmax, err uneven ground
isterminal = [1,1];   % stop the integration
